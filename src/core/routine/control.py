...
split_control = persist.ObjectPersistance()

@split_control.type(persist.SPECIAL_OBJECT)
class SplitControl(SourceRoutine,TargetRoutine):
    ...
    def _process(self,message,ipipe):        
        for opipe in self.target:
            self.processor.schedule(message,ipipe,opipe)
        else:
...
merge_control = persist.ObjectPersistance()

@merge_control.type(persist.SPECIAL_OBJECT)
...
allow_control = persist.ObjectPersistance()

@allow_control.type(persist.CONDITION_OBJECT)
...
block_control = persist.ObjectPersistance()

@block_control.type(persist.CONDITION_OBJECT)
