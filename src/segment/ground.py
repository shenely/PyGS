...
from ..routine import control,queue,socket,epoch,command,acknowledge,result
from ..core.state import BaseState
from ..core.command import ManeuverCommand
#
##################
...
class GroundSegment(object):
    ...
    def task_request_command(self):
        ...
        after_system = epoch.after(self.physics,COMMAND_MARGIN,None,dequeue_cmd)
        before_system = epoch.before(self.physics,DISCARD_MARGIN,remove_cmd,after_system)
    ...
    def task_publish_status(self):
        ...
        format_status = result.format(STATUS_ADDRESS.format(name=self.name),publish_status)
