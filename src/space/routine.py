...
def kepler(state,step=CLOCK_STEP,pipeline=None):
    ...
    logging.debug("Space.Kepler:  Stating at %s" % state.epoch)
    while True:
        try:
            yield message,pipeline
        except GeneratorExit:
            logging.warn("Space.Kepler:  Stopping at %s" % state.epoch)
            ...
            logging.info("Space.Kepler:  Propagated to %s" % state.epoch)